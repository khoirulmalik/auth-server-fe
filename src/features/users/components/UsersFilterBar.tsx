...
export const UsersFilterBar: React.FC<Props> = ({
  ...
  // Show specialization/category filters only when role filter is in the specialization group,
    ...
    <div className="card p-4 space-y-3">
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-3">
        ...
        <select
          ...
          value={state.role}
          onChange={(e) => {
            const nextRoleSupportsSpec =
              e.target.value === "all" ||
              ROLES_WITH_SPECIALIZATION.includes(e.target.value as Role);
            onChange({
              role: e.target.value as Role | "all",
              // Clear specialization/category if new role doesn't support them
              specialization: nextRoleSupportsSpec
                ? state.specialization
                : "all",
              category: nextRoleSupportsSpec ? state.category : "all",
            });
          }}
        >
        ...
        {showSpecializationFilter && (
          <select
            ...
            onChange={(e) =>
              onChange({
                category: e.target.value as EngineerCategory | "all",
              })
            }
          >
